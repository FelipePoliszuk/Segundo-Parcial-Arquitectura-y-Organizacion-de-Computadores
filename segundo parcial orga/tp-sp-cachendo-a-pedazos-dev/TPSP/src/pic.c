...
void pic_reset() {
  outb(PIC1_PORT, 0x11);
  outb(PIC1_PORT + 1, 0x20);      //Offset para que las interrupciones no choquen con las nativas del procesador 
  outb(PIC1_PORT + 1, 0x04);
  outb(PIC1_PORT + 1, 0x01);
  outb(PIC1_PORT + 1, 0xFF);

  outb(PIC2_PORT, 0x11);
  outb(PIC2_PORT + 1, 0x28);      //Offset para que las interrupciones no choquen con las nativas del procesador ni las del PIC1
