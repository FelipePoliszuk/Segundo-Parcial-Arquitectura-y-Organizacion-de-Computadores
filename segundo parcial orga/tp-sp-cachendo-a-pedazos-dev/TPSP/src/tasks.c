 ...
 */
void tasks_syscall_draw(
    ca viewport[TASK_VIEWPORT_HEIGHT][TASK_VIEWPORT_WIDTH]) {
  ...
  if (viewport_addr < TASK_CODE_VIRTUAL)
    return;
  // Si el viewport termina después de la memoria de la tarea ignoramos la
  // syscall NOTA: `sizeof(*viewport)` nos da el tamaño del array :)
  if (TASK_SHARED_PAGE <= viewport_addr + sizeof(*viewport))
    return;
