...
```
Con modo real nos referimos al modo en el que arrancan todos los procesadores x86 despues de un 
power-up o reset. El modo protegido es el estado nativo del procesador. 

En modo real el procesador trabaja en modo 16 bits, mientras que en modo protegido trabaja en modo 32 bits.

En modo real, el procesador tiene acceso a 1MB de memoria, mientras que en modo protegido, el procesador 
tiene acceso a 4GB de memoria.

En modo real, el procesador no tiene privilegios, mientras que en modo protegido si.

El set de instrucciones en modo protegido es mas limitado que en modo real.
...
```
No hay proteccion de memoria, menos direccionamiento, al ser de 16 bits los registros son mas chicos.
```
...
```
La GDT es la Global Descriptor Table, contiene los descriptores de segmento y sus caracteristicas. 

Limit: especifica el tamaño total del segmento.

Base: es la dirección física donde empieza el segmento.

G: interpreta el segmento como unidad de bytes o como unidad de 4-KB.

P: indica si esta presente o  no (set/clear) el segmento en la memoria.

DPL: especifica el nivel de privilegio del segmento.

S: indica si el segmento es de sistema o de codigo/datos
```   
...
```    
La variable extern gdt_entry_t gdt representa una lista de descriptores de segmento y extern 
gdt_descriptor_t GDT_DESC indica la GDTR 
```  
...
```  
lgdt carga en gdtr la direccion de la gdt y su tamaño. La estructura que almacena su direccion y 
el tamaño es GDT_DESC inicializada en gdt.c
```  

11. Completen el archivo `kernel.asm` en la sección de cargar la GDT usando lo averiguado en el punto 8 para cargar la GDT.

```
lgdt carga en gdtr la direccion de la gdt y su tamaño. La estructura que almacena su direccion y el tamaño es GDT_DESC inicializada en gdt.c
```
...
```
Si, deberiamos modificar el bit PE 
```
...
```
Usamos el selector de segmento de código de nivel 0
```
...
```
Tendria 80 (columnas) * 50 (filas) * 2 (bytes de color)
```
...
```
Es el metodo que se encarga de dibujar una caja rellena del caracter y atributo pasado por parametro. 
La estructura que se usa apra representar cada caracter en pantalla es ca y ocupa 2 bytes en memoria
```
