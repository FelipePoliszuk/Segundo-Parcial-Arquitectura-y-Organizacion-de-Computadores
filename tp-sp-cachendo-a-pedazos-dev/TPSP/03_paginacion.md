...
```
Hay 2 niveles, 0 = Supervisor y 1 = User
```
...
```
Dado que estamos en una segmentacion flat, sabemos que la direccion logica es lo mismo que una direccion virtual.

Agarramos los primero 20 bits del CR3, esto nos da la direccion del Page Directory.
Guardas el PD_Index que serian los 10 bits mas significativos de la direccion logica.
Accedes al PD[PD_Index] y esto te daria la direccion base de la Page Table en los 20 bits mas significativos.
Guardas el PT_Index que serian los segundos 10 bits mas significativos (los del medio) de la direccion logica.
Accedes al PT[PT_Index] y esto te daria la direccion base de la Pagina (Page Address) en los 20 bits mas significativos.
Guardas el Offset de la pagina que serian los 12 bits menos significativos de la direccion logica.
Con un OR le agregas a la Page Address el Offset obteniendo la direccion fisica.
```

c)  ¿Cuál es el efecto de los siguientes atributos en las entradas de la
    tabla de página?

  - `D = Dirty, si se modifico la memoria con esta Page Table Entry`
  - `A = Accessed, si se accedio a memoria con esta Page Table Entry `
  - `PCD = Page Cache Disable, la pagina no se guarda en la cache`
  - `PWT = Page Write-Through, deshabilita hacer writeback al escribir en la pagina asociada`
  - `U/S = User / Supervisor, nivel de privilegio`
  - `R/W = Read / Write, lectura o escritura`
  - `P = Present, si no esta en 1 entonces es invalido`
...
```
En el caso del U/S observamos que tiene conducta similar a un OR donde si alguno de ellos 
es Supervisor entonces el efecto combinado es Supervisor, caso contrario quedara en User.

Para el R/W notamos que es como si fuese un AND donde ambos tienen que ser Read-Write para 
que el efecto combinado tambien lo sea. En otro caso quedaria Read-Only.
```
...
```
Necesitarimos 5 paginas. 3 para la memoria de la tarea, 1 para la PT y 1 para la PD
```
...
```
La TLB funciona como una cache de traducciones de paginas. Es importante purgarla al modificar 
paginas para que todo lo que este en la cache sea lo mismo que lo que esta en memoria.

Al usar la TLB se almacena:
-La direccion fisica
-Los derechos de acceso (U/S, R/W)
-Dirty flag y memory type

El borrar algo en la TLB no modifica la memoria original.
```
...
```
Es necesario mapear las paginas ya que una vez activada la paginacion el procesador interpreta 
todas las direcciones como virtuales. Usamos rcr3 ya que en C no se puede obtener directamente el registro cr3.
```
